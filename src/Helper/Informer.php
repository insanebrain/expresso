...
{
    /**
     * @var \Expresso\Console\Output\OutputWatcher
     */
    private $output;
    ...
    /**
     * @param \Expresso\Console\Output\OutputWatcher $output
     */
     ...
     * @param string $result
     */
    public function displayDebug($result)
    {
        if ($result && $this->output->isDebug()) {
            $this->output->writeln(array_map(function ($line) {
                return "\033[1;30m< $line\033[0m";
            }, explode("\n", $result)), OutputInterface::OUTPUT_RAW);
     ...
     */
    public function displayVeryVerbose($command)
    {
        if ($this->output->isVeryVerbose()) {
            $this->output->writeln("<fg=red>></fg=red> $command");
        }
    }
}
